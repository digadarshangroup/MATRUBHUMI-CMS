...
router.get("/filters", EmployeeAuthMiddlewear, async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            company: { id: 1, name: "Matrubhoomi Farms & Developers" },   // placeholder — expand when multi-company
...
//    ┌──────────────────────────────────────────────────────────────┐
//    │ Dept.Name | <dept>      │ CompName | Matrubhoomi │ Report Month    │
...
function renderMonthPerformanceBlock(doc, emp, allDays, monthLabel, L) {
    ...
    // Row 1: Dept. Name | <dept> | CompName | Matrubhoomi | Report Month | <month>
    ...
    drawCell(doc, startX + col1W + col2W + col3W, y, col4W, headerH, "Matrubhoomi", { align: "left", size: 10, bold: true, color: "#7C3AED" });
...
function renderEmpBlockExcel(ws, startRow, emp, allDays, monthLabel, L) {
    ...
    r1.getCell(deptSpan + 2).value = "Matrubhoomi";
