...
function computeDay(
  ...
  if (!hasInAndOut) {
    ...
    // Operator: netWork (excl. breaks): < 6.5h (390m) = HD; >= 6.5h = present
    const opHDMins = shift.halfDayThresholdMins ?? 390;
    if (netWorkMins < opHDMins) {
      systemPrediction = "HD";
    } else if (isLate) {
      systemPrediction = "P*";
    ...
    // Executive: totalSpan (incl. breaks): <= 7.5h (450m) = HD
    const execHDMins = shift.halfDayThresholdMins ?? 450;
    if (totalSpanMins <= execHDMins) {
      systemPrediction = "HD";
    } else if (isLate) {
      systemPrediction = "P*";
    } else if (isEarlyDeparture) {
...
function mergeEmployeeEntry(fresh, existing, shift, isToday) {
  ...
  else if (fresh.employeeType === "operator") {
    const opHDMins = shift.halfDayThresholdMins ?? 390;
    if (netWorkMins < opHDMins) systemPrediction = "HD";
    else if (lateMins > 0) systemPrediction = "P*";
    else if (earlyDepartureMins > 0) systemPrediction = "P~";
    else systemPrediction = "P";
  } else {
    const execHDMins = shift.halfDayThresholdMins ?? 450;
    if (totalSpanMins <= execHDMins) systemPrediction = "HD";
    else if (lateMins > 0) systemPrediction = "P*";
    else if (earlyDepartureMins > 0) systemPrediction = "P~";
    else systemPrediction = "P";
  }
...
function buildSummary(employees) {
  ...
  for (const e of employees) {
    ...
    const countAs =
      st === "LHD" ? "HD" : st === "LAB" || st === "EAB" ? "AB" : st;
    if (s[countAs] !== undefined) s[countAs]++;
...
async function applyLeaveToAttendance(leaveApp) {
  ...
  // Build the per-day status map respecting paidDays vs lwpDays
  const dateStatusMap = buildLeaveDateMap(leaveApp, null);
  ...
  for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
    const ds = dateStrOf(new Date(d));
    const statusCode = dateStatusMap.get(ds); // undefined = rest day, skip
    ...
    if (idx === -1) {
      // Employee not in day doc — only inject if it's a working day
      if (!statusCode) {
        skipped++;
        continue;
      } // rest day, skip
      dayDoc.employees.push({
        ...
        hrRemarks: `Leave approved (${leaveApp.leaveType}${statusCode === "LWP" ? "/LWP" : ""})`,
        hrReviewedAt: new Date(),
      });
      applied++;
    } else {
      const emp = dayDoc.employees[idx];
      // Skip rest days in attendance — don't overwrite WO/holidays
      ...
      if (!statusCode) {
        skipped++;
        continue;
      } // rest day per our map
      emp.hrFinalStatus = statusCode;
      emp.hrRemarks = `Leave approved (${leaveApp.leaveType}${statusCode === "LWP" ? "/LWP" : ""})`;
...
async function applyApprovedLeavesForDate(dateStr) {
  ...
  for (const app of apps) {
    ...
    // Build the full date map for this app, then pick this day's code
    const dateStatusMap = buildLeaveDateMap(app, null);
    const statusCode = dateStatusMap.get(dateStr);
    // If dateStr is a rest day or outside range, statusCode is undefined → skip
    ...
    if (idx === -1) {
      dayDoc.employees.push({
        ...
        systemPrediction: "AB",
        hrFinalStatus: statusCode,
        hrRemarks: `Leave approved (${app.leaveType}${statusCode === "LWP" ? "/LWP" : ""})`,
        hrReviewedAt: new Date(),
      });
      appliedCount++;
      dayDoc.markModified("employees");
    ...
    if (["WO", "FH", "NH", "OH", "RH", "PH"].includes(emp.systemPrediction))
      continue;
    if (emp.hrFinalStatus === statusCode) continue; // already correct
    emp.hrFinalStatus = statusCode;
    emp.hrRemarks =
      emp.hrRemarks ||
      `Leave approved (${app.leaveType}${statusCode === "LWP" ? "/LWP" : ""})`;
    emp.hrReviewedAt = new Date();
    dayDoc.markModified("employees");
    await dayDoc.save();
    appliedCount++;
...
async function applyMonthlyLatePromotion(dayDoc, settings) {
  ...
  for (const d of allDays) {
    const isTarget = d.dateStr === dayDoc.dateStr;
    for (const e of d.employees || []) {
      const key = e.employeeDbId?.toString() || e.biometricId;
      if (!key) continue;
      if (!running.has(key)) running.set(key, { lateCount: 0, earlyCount: 0 });
      const state = running.get(key);
      const { promotedStatus, promoted } = applyLateCountPromotion(
        e,
        state,
        policy,
        d.dateStr,
  ...
  return (dayDoc.employees || []).map((e) => {
    ...
    const systemPrediction = info.promotedStatus || e.systemPrediction;
    return {
      ...e,
      lateCount: info.lateCount,
      earlyCount: info.earlyCount,
      wasPromotedToHalfDay: info.promoted,
      systemPrediction,
...
async function getDailyAttendance(date, department) {
  ...
  const settings = await AttendanceSettings.getConfig();
    ...
    const restDayStatus = resolveRestDayStatus(date, dayOfWeek, holidayMap);
    // Only inject on working days (not WO/holiday — those are already injected by sync)
    if (!restDayStatus) {
      ...
      for (const emp of allActive) {
        ...
        if (
          department &&
          department !== "all" &&
          extractDepartment(emp) !== department
        )
          continue;
        const empType = resolveEmployeeType(emp, settings);
        const shift = shiftFor(emp, settings);
      ...
      // Clone dayDoc to avoid mutating the lean object
      dayDoc = {
        ...dayDoc,
        employees: [...(dayDoc.employees || []), ...absentEntries],
      };
    }
  ...
  try {
    ...
    for (const d of days) {
      ...
      for (const e of d.employees || []) {
        ...
        const _state_s = running.get(key) || { lateCount: 0, earlyCount: 0 };
        const { promotedStatus: _ps_s, promoted } = applyLateCountPromotion(
          e,
          _state_s,
          settings.lateHalfDayPolicy,
          d.dateStr,
          todayStr,
        );
        running.set(key, _state_s);
        let status = _ps_s || e.systemPrediction;
...
router.get("/employee-detail", EmployeeAuthMiddlewear, async (req, res) => {
  try {
    ...
    const settings = await AttendanceSettings.getConfig();
    const _nIST_ed = new Date(Date.now() + 330 * 60 * 1000);
    const todayStr = `${_nIST_ed.getUTCFullYear()}-${String(_nIST_ed.getUTCMonth() + 1).padStart(2, "0")}-${String(_nIST_ed.getUTCDate()).padStart(2, "0")}`;
    ...
    for (const cal of calendar) {
      ...
      const _state_ed2 = running.get(bid) || { lateCount: 0, earlyCount: 0 };
      const { promotedStatus: _ps_ed2, promoted } = applyLateCountPromotion(
        entry,
        _state_ed2,
        settings.lateHalfDayPolicy,
        cal.dateStr,
      ...
      running.set(bid, _state_ed2);
      let status = _ps_ed2 || entry.systemPrediction;
  ...
  try {
    ...
    for (const emp of filteredActive) {
      ...
      for (const cal of allDays) {
            ...
            const _mr_countAs =
              finalStatus === "LHD"
                ...
                : finalStatus === "LAB" || finalStatus === "EAB"
                  ? "AB"
                  : finalStatus;
            if (row.totals[_mr_countAs] !== undefined)
              row.totals[_mr_countAs]++;
        ...
        const _st_ek = running.get(key) || { lateCount: 0, earlyCount: 0 };
        const { promotedStatus: _ps_ek, promoted } = applyLateCountPromotion(
          entry,
          _st_ek,
          settings.lateHalfDayPolicy,
          cal.dateStr,
        ...
        running.set(key, _st_ek);
        let status = _ps_ek || entry.systemPrediction;
        const finalStatus = entry.hrFinalStatus || status;
        row.days[cal.dateStr] = {
          status: finalStatus,
...
router.get("/export-daily", EmployeeAuthMiddlewear, async (req, res) => {
  try {
    ...
    wb.creator = "Matrubhoomi Farms & Developers HRMS";
    ...
    hero.value = {
      richText: [
        {
          text: "MATRUBHOOMI FARMS & DEVELOPERS\n",
          font: {
            size: 11,
            bold: true,
            italic: true,
    ...
    ft.value = `Generated ${new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })}  ·  Matrubhoomi Farms & Developers HRMS  ·  Daily Manpower Report`;
...
router.get("/export-muster-roll", EmployeeAuthMiddlewear, async (req, res) => {
  try {
    ...
    if (_exportPolicy?.enabled) {
      ...
      for (const doc of dayDocs) {
        ...
        for (const e of doc.employees || []) {
          ...
          if (!_bid) continue;
          if (!_running.has(_bid))
            _running.set(_bid, { lateCount: 0, earlyCount: 0 });
    ...
    function buildEmpRow(key, empName, empDept, empDesig, empType) {
      ...
      for (const cal of allDays) {
        ...
        if (cal.isDeclaredHoliday) {
          ...
          const _promoStatus = promotionMap
            .get(String(key).toUpperCase())
            ?.get(cal.dateStr);
          const finalStatus =
            entry.hrFinalStatus || _promoStatus || entry.systemPrediction;
          ...
          if (["P", "P*", "P~", "MP", "WFH"].includes(finalStatus))
            ...
            row.totals.A++;
          else if (finalStatus === "HD" || finalStatus === "LHD")
            row.totals.HD++;
    ...
    const STATUS_STYLE = {
      ...
      LHD: { font: "FF7D4701", fill: "FFFFEB9C" },
      LAB: { font: "FF9C0006", fill: "FFFFC7CE" },
      EAB: { font: "FF9C0006", fill: "FFFFC7CE" },
...
router.get("/timecard", EmployeeAuthMiddlewear, async (req, res) => {
  try {
    ...
    for (const cal of calendar) {
      ...
      const _state_tc = running.get(bid) || { lateCount: 0, earlyCount: 0 };
      const { promotedStatus: _ps_tc, promoted } = applyLateCountPromotion(
        entry,
        _state_tc,
        settings.lateHalfDayPolicy,
        cal.dateStr,
      ...
      running.set(bid, _state_tc);
      let status = _ps_tc || entry.systemPrediction;
      const finalStatus = entry.hrFinalStatus || status;
