...
function buildInternStipendSheet(wb, items, { month, year }) {
  ...
  title.value = `MATRUBHOOMI FARMS & DEVELOPERS  ·  Intern Stipends  ·  ${MONTH_NAMES[month]} ${year}`;
...
router.get("/export", EmployeeAuthMiddlewear, async (req, res) => {
  try {
    ...
    if (segment === "interns") {
      ...
      internWb.creator = "Matrubhoomi Farms & Developers HRMS";
    ...
    r1.value = `MATRUBHOOMI FARMS & DEVELOPERS  ·  Salary Sheet  ·  ${MONTH_NAMES[month]} ${year}`;
    r1.font = {
      name: "Arial",
      size: 13,
      bold: true,
