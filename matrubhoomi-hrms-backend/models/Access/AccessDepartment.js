...
const accessDepartmentSchema = new mongoose.Schema(
  {
    ...
    // Reserved. A department that lives in a SEPARATE application on its own
    // origin would need a real base URL here rather than a Next.js path, plus
    // a single-sign-on bridge to hand the browser off with. Neither exists in
    // this deployment — every department is a route inside the one app — so
    // nothing reads this today. Kept as a field rather than deleted so an
    // existing row that carries a value does not silently lose it.
...
accessDepartmentSchema.methods.toPublicTile = function () {
  return {
    ...
    // Reserved — see the field's own note above. Nothing in this deployment
    // reads it; it is projected so a client that does would not have to fetch
    // the department a second time. Not sensitive: it is the
